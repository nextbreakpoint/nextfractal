...
public class JXTARelayService {
	...
	private static final ThreadFactory factory = new DefaultThreadFactory("JXTARelay", true, Thread.MIN_PRIORITY);
		...
		public void run() {
			...
			try {
				...
				if (!manager.waitForRendezvousConnection(20000)) {
					logger.warning("Failed to connect to relay service");
				}
				while (running) {
					Thread.sleep(60000);
				}
