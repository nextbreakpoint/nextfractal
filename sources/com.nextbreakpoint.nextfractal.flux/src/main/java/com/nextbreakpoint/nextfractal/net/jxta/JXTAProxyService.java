...
public class JXTAProxyService {
	...
	private static void configureNetwork(final NetworkManager manager) throws Exception {
		...
		if (System.getProperty(JXTANetworkService.NextFractal_JXTA_RELAY_SEEDS) != null) {
			URI seedingURI = new File(System.getProperty(JXTANetworkService.NextFractal_JXTA_RELAY_SEEDS)).toURI();
			logger.info("Add relay seeds: " + seedingURI);
			config.clearRelaySeedingURIs();
			config.addRelaySeedingURI(seedingURI);
		}
		if (System.getProperty(JXTANetworkService.NextFractal_JXTA_RENDEZVOUS_SEEDS) != null) {
			URI seedingURI = new File(System.getProperty(JXTANetworkService.NextFractal_JXTA_RENDEZVOUS_SEEDS)).toURI();
			logger.info("Add rendezvous seeds: " + seedingURI);
			config.clearRendezvousSeedingURIs();
			config.addRdvSeedingURI(seedingURI);
		}
		if (Boolean.getBoolean(JXTANetworkService.NextFractal_JXTA_MULTICAST)) {
