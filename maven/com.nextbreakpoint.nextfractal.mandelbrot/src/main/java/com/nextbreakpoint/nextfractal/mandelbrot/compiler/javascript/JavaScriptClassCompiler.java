...
public class JavaScriptClassCompiler {
	...
	public CompilerBuilder<Orbit> compileOrbit(CompilerReport report) throws ClassNotFoundException, IOException {
		...
		return new JavaScriptOrbitBuilder(report.getAST(), report.getOrbitContext(), report.getOrbitSource(), errors);
	}

	public CompilerBuilder<Color> compileColor(CompilerReport report) throws ClassNotFoundException, IOException {
		List<CompilerError> errors = new ArrayList<>();
		return new JavaScriptColorBuilder(report.getAST(), report.getColorContext(), report.getColorSource(), errors);
	}
}	
