...
public class InterpreterOperatorMul implements CompiledExpression {
	...
	public InterpreterOperatorMul(ExpressionContext context, CompiledExpression exp1, CompiledExpression exp2) {
	...
	public double evaluateReal(InterpreterContext context) {
		return opMul(exp1.evaluateReal(context), exp2.evaluateReal(context));
	}

	@Override
	public Number evaluate(InterpreterContext context) {
		return opMul(context.getNumber(index), exp1.evaluate(context), exp2.evaluateReal(context));
	}
