...
void run_java(struct launch_args *run_args) {
    ...
    std::regex path_regex("(jdk1.[0-9]+\\.[0-9]+_[0-9]+)|(jdk-[0-9]+\\.[0-9]+\\.[0-9]+)|(jdk-[0-9])"", std::regex_constants::ECMAScript | std::regex_constants::icase);
    ...
    if (NULL == jniModule) {
        throw std::runtime_error("Cannot open library jvm.dll");
    }
