...
public class BrowseListCell extends ListCell<ContextFreeData[]> {
	private HBox pane;
	...
	@Override
	public void updateItem(ContextFreeData[] files, boolean empty) {
		super.updateItem(files, empty);
		if (empty) {
			setGraphic(null);
		} else {
			for (int i = 0 ; i < numOfColumns; i++) {
				ContextFreeData data = files[i];
	...
	private void renderFractal(GraphicsContext g2d, ContextFreeData data) {
