...
class ASTRule extends ASTReplacement implements Comparable<ASTRule> {
	...
	public ASTRule(CFDGDriver driver, int nameIndex, float weight, boolean percent, Token location) {
		super(driver, null, ERepElemType.rule, location);
		...
		if (nameIndex != EPrimShape.circleType.getType()) {
			...
			while (!isStop(cmd = iterator.currentSegment(coords))) {
				if (isVertex(cmd)) {
					...
					ASTPathOp op = new ASTPathOp(driver, isMoveTo(cmd) ? EPathOp.MOVETO.name() : EPathOp.LINETO.name(), a, location);
					getRuleBody().getBody().add(op);
				}
			}
		} else {
			ASTExpression a = new ASTCons(location, new ASTReal(0.5, location), new ASTReal(0.0, location));
			ASTPathOp op = new ASTPathOp(driver, EPathOp.MOVETO.name(), a, location);
			getRuleBody().getBody().add(op);
			a = new ASTCons(location, new ASTReal(-0.5, location), new ASTReal(0.0, location), new ASTReal(0.5, location));
			op = new ASTPathOp(driver, EPathOp.ARCTO.name(), a, location);
			getRuleBody().getBody().add(op);
			a = new ASTCons(location, new ASTReal(0.5, location), new ASTReal(0.0, location), new ASTReal(0.5, location));
			op = new ASTPathOp(driver, EPathOp.ARCTO.name(), a, location);
			getRuleBody().getBody().add(op);
		}
		getRuleBody().getBody().add(new ASTPathOp(driver, EPathOp.CLOSEPOLY.name(), null, location));
